...
#     log.info(f"Assembling {len(blocks)} blocks into chunks of size {chunk_size} with overlap {overlap}.")
...
#                 log.info(f"Added block {i} to current chunk (length={current_length}).")
#                 i += 1
#             else:                  
#                 # finalize the current chunk when it reaches the chunk size limit
...
#                 log.info(f"Finalized chunk of length {current_length} with section title '{metadata['section_title']}'.")
...
#                 if current_length + block_length > chunk_size:
#                     log.info(f"Block too large to fit into overlap chunk (len={block_length}), forcing as single chunk.")
#                     current_chunk = [(block, section)]
...
#         return chunks
#     except Exception as e:
#         log.error(f"Error assembling chunks from blocks: {e}", exc_info=True)
#         return []
